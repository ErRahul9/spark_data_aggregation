...
def spark_runner_hrly(date,time,s3_output_path):
    spark = spark_connection()
    S3_path = f's3a://mntn-data-archive-prod/bid_price_log_v2/dt={date.split("=")[1]}/hh={time.split("=")[1]}'
...
if __name__ == "__main__":
    ...
    time = args.hour
    s3_output_path = args.output_location
    spark_runner_hrly(date,time,s3_output_path)
